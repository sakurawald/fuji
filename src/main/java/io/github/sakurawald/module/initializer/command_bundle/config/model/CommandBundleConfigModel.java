    ...
    List<BundleCommandEntry> entries = new ArrayList<>() {
        {
            ...
            this.add(new BundleCommandEntry(new CommandRequirementDescriptor(4, null), "gm <gamemode gamemode-arg>", List.of("run as player %player:name% gamemode $gamemode-arg")));
            this.add(new BundleCommandEntry(new CommandRequirementDescriptor(4, null), "unbreakable", List.of("run as player %player:name% enchant %player:name% minecraft:unbreaking")));

            this.add(new BundleCommandEntry(new CommandRequirementDescriptor(4, null), "move-speed set <double double-arg>", List.of("run as player %player:name% attribute %player:name% minecraft:generic.movement_speed base set $double-arg")));
            this.add(new BundleCommandEntry(new CommandRequirementDescriptor(4, null), "move-speed reset", List.of("run as player %player:name% attribute %player:name% minecraft:generic.movement_speed base set 0.10000000149011612")));

            this.add(new BundleCommandEntry(new CommandRequirementDescriptor(4, null), "warn <player player-arg> <greedy greedy-arg>", List.of("run as player %player:name% send-message $player-arg <red>You are warned: $greedy-arg")));
