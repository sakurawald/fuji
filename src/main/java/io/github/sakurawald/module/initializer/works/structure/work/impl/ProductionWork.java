...
public class ProductionWork extends Work implements Schedulable {
    ...
    public @NotNull List<Text> asLore(ServerPlayerEntity player) {
        ...
        if (this.sample.sampleCounter != null) {
            // trim counter
            if (this.sample.sampleCounter.size() > Configs.configHandler.getModel().modules.works.sample_counter_top_n) {
                trimCounter();
            }
    ...
    public void openInputSampleDistanceGui(@NotNull ServerPlayerEntity player) {
        new InputSignGui(player,  "works.production_work.prompt.input.sample_distance") {
            @Override
            public void onClose() {
                int limit = Configs.configHandler.getModel().modules.works.sample_distance_limit;
                int current;
                try {
    ...
    public void startSample(@NotNull ServerPlayerEntity player) {
        ...
        this.sample.sampleEndTimeMS = this.sample.sampleStartTimeMS + Configs.configHandler.getModel().modules.works.sample_time_ms;
    ...
    public void trimCounter() {
        ...
        int N = Configs.configHandler.getModel().modules.works.sample_counter_top_n;
