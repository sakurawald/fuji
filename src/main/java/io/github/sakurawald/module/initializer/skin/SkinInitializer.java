...
public class SkinInitializer extends ModuleInitializer {
    ...
    private int skinAction(@NotNull ServerCommandSource src, @NotNull Collection<GameProfile> targets, boolean setByOperator, @NotNull Supplier<Property> skinSupplier) {
        SkinRestorer.setSkinAsync(src.getServer(), targets, skinSupplier).thenAccept(pair -> {
            ...
            if (profiles.isEmpty()) {
                MessageHelper.sendMessageByKey(src, "skin.action.failed");
                return;
            }
            if (setByOperator) {
                MessageHelper.sendMessageByKey(src, "skin.action.affected_profile", String.join(", ", profiles.stream().map(GameProfile::getName).toList()));
                if (!players.isEmpty()) {
                    MessageHelper.sendMessageByKey(src, "skin.action.affected_player", String.join(", ", players.stream().map(p -> p.getGameProfile().getName()).toList()));
                }
            } else {
                MessageHelper.sendMessageByKey(src, "skin.action.ok");
            }
        });
        return targets.size();
    }
