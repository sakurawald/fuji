...
public class AddWorkGui extends InputSignGui {
    ...
    public void onClose() {
        ...
        selectWorkTypeGui.setTitle(LanguageHelper.getTextByKey(player, "works.work.add.select_work_type.title"));
        ...
        selectWorkTypeGui.setSlot(11, new GuiElementBuilder().setItem(Items.GUNPOWDER).setName(LanguageHelper.getTextByKey(player, "works.non_production_work.name")).setCallback(() -> {
            // add
            worksHandler.model().works.addFirst(new NonProductionWork(player, name));
            LanguageHelper.sendActionBarByKey(player, "works.work.add.done");
            LanguageHelper.sendBroadcastByKey("works.work.add.broadcast", player.getGameProfile().getName(), name);
            selectWorkTypeGui.close();
        }));

        selectWorkTypeGui.setSlot(15, new GuiElementBuilder().setItem(Items.REDSTONE).setName(LanguageHelper.getTextByKey(player, "works.production_work.name")).setCallback(() -> {
            // add
