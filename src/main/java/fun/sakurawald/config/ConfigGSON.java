package fun.sakurawald.config;


import com.mojang.authlib.properties.Property;
import net.fabricmc.loader.api.FabricLoader;
import net.minecraft.world.entity.EntityType;
import net.minecraft.world.level.block.entity.BlockEntityType;

import java.util.ArrayList;
import java.util.HashMap;
import java.util.List;

@SuppressWarnings("ALL")
public class ConfigGSON {

    public Modules modules = new Modules();

    public class Modules {

        public ResourceWorld resource_world = new ResourceWorld();
        public NewbieWelcome newbie_welcome = new NewbieWelcome();
        public TeleportWarmup teleport_warmup = new TeleportWarmup();
        public DynamicMOTD dynamic_motd = new DynamicMOTD();
        public BetterFakePlayer better_fake_player = new BetterFakePlayer();
        public CommandCooldown command_cooldown = new CommandCooldown();
        public TopChunks top_chunks = new TopChunks();
        public ChatStyle chat_style = new ChatStyle();
        public Skin skin = new Skin();
        public Back back = new Back();
        public Tpa tpa = new Tpa();
        public Works works = new Works();
        public WorldDownloader world_downloader = new WorldDownloader();
        public Display display = new Display();

        public class ResourceWorld {
            public long seed = 0L;
        }

        public class DynamicMOTD {
            public List<String> descriptions = new ArrayList<>() {
                {
                    this.add("<gradient:#FF66B2:#FFB5CC>Pure Survival 1.20.1 / Up %uptime%H ❤ QQ Group 912363929</gradient><newline><gradient:#99CCFF:#BBDFFF>%playtime%🔥 %mined%⛏ %placed%🔳 %killed%🗡 %moved%🌍");
                }
            };
        }

        public class NewbieWelcome {
            public RandomTeleport random_teleport = new RandomTeleport();

            public class RandomTeleport {
                public int max_try_times = 32;
                public int min_distance = 5000;
                public int max_distance = 40000;
            }
        }

        public class TeleportWarmup {
            public int warmup_second = 3;
            public double interrupt_distance = 1d;
        }

        public class BetterFakePlayer {
            public ArrayList<List<Integer>> limit_rule = new ArrayList<>() {
                {
                    this.add(List.of(1, 0, FabricLoader.getInstance().isDevelopmentEnvironment() ? 9999 : 2));
                }
            };

            public int renew_duration_ms = 1000 * 60 * 60 * 12;
            public String transform_name = "_fake_%name%";
        }

        public class CommandCooldown {
            public HashMap<String, Long> command_regex_2_cooldown_ms = new HashMap<>() {
                {
                    this.put("rw tp (overworld|the_nether|the_end)", 120 * 1000L);
                    this.put("chunks\\s*", 60 * 1000L);
                    this.put("download\\s*", 300 * 1000L);
                }
            };
        }

        public class TopChunks {

            public int rows = 10;
            public int columns = 10;
            public int nearest_distance = 128;

            public boolean hide_location = true;
            public HashMap<String, Integer> type2score = new HashMap<>() {
                {
                    this.put("default", 1);
                    this.put(BlockEntityType.getKey(BlockEntityType.CHEST).toLanguageKey("block"), 1);
                    this.put(BlockEntityType.getKey(BlockEntityType.TRAPPED_CHEST).toLanguageKey("block"), 2);
                    this.put(BlockEntityType.getKey(BlockEntityType.BARREL).toLanguageKey("block"), 1);
                    this.put(BlockEntityType.getKey(BlockEntityType.FURNACE).toLanguageKey("block"), 3);
                    this.put(BlockEntityType.getKey(BlockEntityType.BLAST_FURNACE).toLanguageKey("block"), 4);
                    this.put(BlockEntityType.getKey(BlockEntityType.SMOKER).toLanguageKey("block"), 3);
                    this.put(BlockEntityType.getKey(BlockEntityType.JUKEBOX).toLanguageKey("block"), 35);
                    this.put(BlockEntityType.getKey(BlockEntityType.BEACON).toLanguageKey("block"), 35);
                    this.put(BlockEntityType.getKey(BlockEntityType.CONDUIT).toLanguageKey("block"), 40);
                    this.put(BlockEntityType.getKey(BlockEntityType.HOPPER).toLanguageKey("block"), 8);
                    this.put(BlockEntityType.getKey(BlockEntityType.PISTON).toLanguageKey("block"), 10);
                    this.put(BlockEntityType.getKey(BlockEntityType.DISPENSER).toLanguageKey("block"), 10);
                    this.put(BlockEntityType.getKey(BlockEntityType.DROPPER).toLanguageKey("block"), 10);
                    this.put(BlockEntityType.getKey(BlockEntityType.COMPARATOR).toLanguageKey("block"), 5);
                    this.put(BlockEntityType.getKey(BlockEntityType.DAYLIGHT_DETECTOR).toLanguageKey("block"), 25);
                    this.put(BlockEntityType.getKey(BlockEntityType.BEEHIVE).toLanguageKey("block"), 15);
                    this.put(BlockEntityType.getKey(BlockEntityType.MOB_SPAWNER).toLanguageKey("block"), 100);
                    this.put(BlockEntityType.getKey(BlockEntityType.COMMAND_BLOCK).toLanguageKey("block"), 9999);
                    this.put(BlockEntityType.getKey(BlockEntityType.STRUCTURE_BLOCK).toLanguageKey("block"), 9999);
                    this.put(BlockEntityType.getKey(BlockEntityType.JIGSAW).toLanguageKey("block"), 9999);
                    this.put(EntityType.getKey(EntityType.PLAYER).toLanguageKey(), 15);
                    this.put(EntityType.getKey(EntityType.FALLING_BLOCK).toLanguageKey(), 10);
                    this.put(EntityType.getKey(EntityType.ZOMBIE).toLanguageKey(), 4);
                    this.put(EntityType.getKey(EntityType.ZOMBIE_VILLAGER).toLanguageKey(), 8);
                    this.put(EntityType.getKey(EntityType.ZOMBIFIED_PIGLIN).toLanguageKey(), 5);
                    this.put(EntityType.getKey(EntityType.ZOGLIN).toLanguageKey(), 8);
                    this.put(EntityType.getKey(EntityType.PILLAGER).toLanguageKey(), 10);
                    this.put(EntityType.getKey(EntityType.RAVAGER).toLanguageKey(), 4);
                    this.put(EntityType.getKey(EntityType.EVOKER).toLanguageKey(), 5);
                    this.put(EntityType.getKey(EntityType.VEX).toLanguageKey(), 5);
                    this.put(EntityType.getKey(EntityType.PIGLIN).toLanguageKey(), 2);
                    this.put(EntityType.getKey(EntityType.DROWNED).toLanguageKey(), 2);
                    this.put(EntityType.getKey(EntityType.GUARDIAN).toLanguageKey(), 6);
                    this.put(EntityType.getKey(EntityType.SPIDER).toLanguageKey(), 2);
                    this.put(EntityType.getKey(EntityType.SKELETON).toLanguageKey(), 2);
                    this.put(EntityType.getKey(EntityType.CREEPER).toLanguageKey(), 3);
                    this.put(EntityType.getKey(EntityType.ENDERMITE).toLanguageKey(), 5);
                    this.put(EntityType.getKey(EntityType.ENDERMAN).toLanguageKey(), 4);
                    this.put(EntityType.getKey(EntityType.WITHER).toLanguageKey(), 55);
                    this.put(EntityType.getKey(EntityType.VILLAGER).toLanguageKey(), 25);
                    this.put(EntityType.getKey(EntityType.SHEEP).toLanguageKey(), 5);
                    this.put(EntityType.getKey(EntityType.COW).toLanguageKey(), 3);
                    this.put(EntityType.getKey(EntityType.MOOSHROOM).toLanguageKey(), 3);
                    this.put(EntityType.getKey(EntityType.CHICKEN).toLanguageKey(), 3);
                    this.put(EntityType.getKey(EntityType.PANDA).toLanguageKey(), 5);
                    this.put(EntityType.getKey(EntityType.WOLF).toLanguageKey(), 8);
                    this.put(EntityType.getKey(EntityType.CAT).toLanguageKey(), 8);
                    this.put(EntityType.getKey(EntityType.BEE).toLanguageKey(), 15);
                    this.put(EntityType.getKey(EntityType.BOAT).toLanguageKey(), 5);
                    this.put(EntityType.getKey(EntityType.CHEST_BOAT).toLanguageKey(), 5);
                    this.put(EntityType.getKey(EntityType.ITEM_FRAME).toLanguageKey(), 3);
                    this.put(EntityType.getKey(EntityType.GLOW_ITEM_FRAME).toLanguageKey(), 3);
                    this.put(EntityType.getKey(EntityType.ARMOR_STAND).toLanguageKey(), 10);
                    this.put(EntityType.getKey(EntityType.ITEM).toLanguageKey(), 2);
                    this.put(EntityType.getKey(EntityType.EXPERIENCE_ORB).toLanguageKey(), 3);
                    this.put(EntityType.getKey(EntityType.TNT).toLanguageKey(), 70);
                    this.put(EntityType.getKey(EntityType.HOPPER_MINECART).toLanguageKey(), 20);
                }
            };
        }

        public class ChatStyle {
            public String format = "<#B1B2FF>[%playtime%\uD83D\uDD25 %mined%⛏ %placed%\uD83D\uDD33 %killed%\uD83D\uDDE1 %moved%\uD83C\uDF0D]<reset> <<dark_green><click:suggest_command:/msg %player% ><hover:show_text:\"Time: <date:'yyyy-MM-dd HH:mm:ss'><newline><italic>Click to Message\">%player%</hover></click></dark_green>> %message%";
            public MentionPlayer mention_player = new MentionPlayer();

            public History history = new History();

            public class History {
                public int cache_size = 50;
            }

            public class MentionPlayer {
                public String sound = "entity.experience_orb.pickup";
                public float volume = 100f;
                public float pitch = 1f;

                public int limit = 3;
                public int interval = 1000;
            }
        }

        public class Skin {

            public ArrayList<Property> default_skins = new ArrayList<>() {
                {
                    this.add(new Property("textures", "", ""));
                    this.add(new Property("textures", "", ""));
                    this.add(new Property("textures", "", ""));
                    this.add(new Property("textures", "", ""));
                    this.add(new Property("textures", "", ""));
                    this.add(new Property("textures", "", ""));
                    this.add(new Property("textures", "", ""));
                    this.add(new Property("textures", "", ""));
                    this.add(new Property("textures", "", ""));
                    this.add(new Property("textures", "", ""));
                    this.add(new Property("textures", "", ""));
                    this.add(new Property("textures", "", ""));
                    this.add(new Property("textures", "", ""));
                    this.add(new Property("textures", "", ""));
                    this.add(new Property("textures", "", ""));
                    this.add(new Property("textures", "", ""));
                    this.add(new Property("textures", "", ""));
                    this.add(new Property("textures", "", ""));
                    this.add(new Property("textures", "", ""));
                    this.add(new Property("textures", "", ""));
                    this.add(new Property("textures", "", ""));
                    this.add(new Property("textures", "", ""));
                }
            };
        }

        public class Back {
            public double ignore_distance = 32d;
        }

        public class Tpa {
            public int timeout = 300;
        }

        public class Works {

            public int sampleTimeMS = FabricLoader.getInstance().isDevelopmentEnvironment() ? 60 * 1000 : 60 * 1000 * 60;
            public int sampleDistanceLimit = 512;
            public int sampleCounterTopN = 20;
        }

        public class WorldDownloader {
            public int port = 22222;
            public int bytes_per_second_limit = 128 * 1000;
            public int context_cache_size = 5;
        }

        public class Display {
            public int expiration_duration_s = 3600;
        }
    }
}
