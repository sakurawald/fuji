package fun.sakurawald.module.profiler;
...
public class ProfilerModule extends AbstractModule {
    ...
    private int $profiler(CommandContext<CommandSourceStack> ctx) {
        ...
        CompletableFuture.runAsync(() -> {
            ...
            if (spark == null) {
                sendMessage(source, "profiler.spark.no_instance");
                return;
            }
            ...
            Component gcComponent = ofComponent(source, "profiler.format.gc.head");
            int i = 0;
            for (GarbageCollector garbageCollector : gc.values()) {
                ...
                gcComponent = gcComponent.append(ofComponent(source, i == gc.values().size() - 1 ? "profiler.format.gc.last" : "profiler.format.gc.no_last", name, avgFrequency, avgTime, totalCollections, totalTime));
                i++;
            }

            Component memComponent = ofComponent(source, "profiler.format.mem.head");
            ...
            for (MemoryPoolMXBean memoryPoolMXBean : memoryPoolMXBeans) {
                ...
                memComponent = memComponent.append(ofComponent(source, i == memoryPoolMXBeans.size() - 1 ? "profiler.format.mem.last" : "profiler.format.mem.no_last", name, type, init, used, committed, max));
                i++;
            }

            /* output */
            Component formatComponent = ofComponent(source, "profiler.format"
