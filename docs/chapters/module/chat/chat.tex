...
\begin{enumerate}
    ...
    \item You can insert any \ttt{placeholder} like \ttt{\ph{world:name}}
    \item You can insert player's prefix and suffix.Just insert \ph{fuji:player\_prefix} and \ph{fuji:player\_suffix}.
    \item You can insert \tbf{item}, \tbf{inv} and \tbf{ender} to display your item, inventory and ender-chest.
    \item You can insert \tbf{Steve} to mention another player named Steve.
    \item You can insert \tbf{pos} to show the position.
...
\begin{note}{Set prefix and suffix for players}
    ...
    \\
    \\
    After you installed \ttt{luckperms} mod, just issue \ttt{/lp group default meta setprefix <yellow>[awesome]} to assign prefix.
    \\
    Don't forget to change the format in chat module, and issue \ttt{/fuji reload}
\end{note}

\begin{tips}{Write complex style using mini-language}
    You can use \ttt{mini-language} to write complex style Text.
    \\
    \\
    See more:
    \begin{enumerate}
